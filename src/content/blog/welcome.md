---
title: "Welcome to My Portfolio / 欢迎来到我的个人主页"
excerpt: "Why I created this space for cyber security projects and learning notes. / 创建这个网络安全个人空间的原因。"
publishDate: 'Jul 12 2026'
isFeatured: true
tags: ["Portfolio", "Introduction"]
---

<section class="lang-en">
**IntroDuction**
<p>Welcome to my personal website. I'm TaeHyun Kim, and I'm currently majoring Bachelor of Coumputer Science(Cyber Security) at the University of Wollongong. Also I'm keep developing my chinese skills own my own as a hobby. Even my chinese skills have far way to go, since I got some interests in studying or working in China, I decided to present this website using both English and chinese.</p>
<p>I will use this website to record my progress of learning, share projects, technical notes, and milestones etc. </p>


<h2>Buiding the site</h2>
<p>This portfolio website is built with Astro.js and Tailwind CSS. Since this is my first time building a website like this, there is still a great deal for me to learn. I plan to continue improving it step by step as I gain more experience and deepen my understanding of modern web development.</p>
<p>Analyzing the template code, removing unnecessary components, fixing errors, and customizing it to suit my own needs was unfamiliar at first. However, as someone who frequently contributes to and edits Wikipedia, I found the process surprisingly engaging and enjoyable. I look forward to continuing to refine both this website and my skills over time.</p>

<h2>Contents Posted Here</h2>
<p>
This website documents my academic journey, personal projects, language learning, and international experiences. I plan to gradually add new content as I continue learning and exploring new opportunities.
</p>

<ul>
  <li><strong>Study Notes:</strong> Notes and reflections from university courses and independent study.</li>
  <li><strong>Projects:</strong> Progress updates, challenges, and lessons learned from academic and personal projects.</li>
  <li><strong>Chinese Language Learning:</strong> My progress, study methods, useful resources, and reflections on learning Chinese.</li>
  <li><strong>International Experiences:</strong> Experiences and reflections from overseas study programs, academic visits, and cultural exchanges.</li>
  <li><strong>Development Log:</strong> Notes on coding, website development, debugging, and technical problem-solving.</li>
  <li><strong>Personal Reflections:</strong> Thoughts on learning, future goals, and experiences that contribute to my personal and academic growth.</li>
</ul>



<h2>In closing</h2>
<p><h2>Final Thoughts</h2>

<p>
Thank you for taking the time to visit my website. I am still learning and exploring, so this site will continue to grow and change over time. I look forward to sharing more of my studies, projects, language-learning journey, and international experiences here.
</p>


</section>


<section class="lang-zh" lang="zh-CN">
<p>欢迎来到我的个人主页。我创建这个网站，是为了记录自己在伍伦贡大学学习计算机科学（网络安全）的成长过程。</p>
<p>这里将用于分享已经完成的项目、技术笔记与学习里程碑。相关内容会在准备完善后陆续发布。</p>
</section>
